...
// Tambahkan badge untuk beberapa kue
kueList.forEach((kue, index) => {
    kue.badge = index % 2 === 0 ? "Best Seller" : "Baru";
});
...
kueList.forEach(kue => {
    ...
    card.addEventListener("click", () => {
        openModal(kue.nama, kue.gambar, kue.deskripsi, waLink);
    });
...
l
function openModal(nama, gambar, deskripsi, waLink) {
    const modal = document.getElementById("modal");
    document.getElementById("modal-title").textContent = nama;
    document.getElementById("modal-image").src = gambar;
    document.getElementById("modal-description").textContent = deskripsi;
    document.getElementById("modal-wa").href = waLink;
